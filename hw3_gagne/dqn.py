...
def learn(env,
    ...
    # squared error loss
    #loss_sq = tf.square(td_error)

    # huber loss (??)
    #loss_huber = tf.losses.huber_loss(q_val_t_selected,q_val_t_selected_target)
    ...
    for t in itertools.count():
        ...
        # if episode done before end of iter (is this the right place)
        if done:
            obs = env.reset()
            # so now on the next iteration of the loop,
            #it'll get a fresh observation to generate an action for

        # update the latest obs,
        last_obs = obs.copy()
